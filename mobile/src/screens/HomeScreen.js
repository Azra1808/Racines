...
// Le pictogramme "Hors connexion" a été retiré de cette rangée : ce n'était
// qu'une puce décorative non cliquable, redondante avec le vrai bouton
// « Canaux hors ligne » ci-dessous, qui lui est cliquable et mène au
// simulateur SMS/USSD/vocal.
export default function HomeScreen({ navigation }) {
  const { colors } = useTheme();
  const { t } = useLanguage();
  // Défini dans le composant : les libellés doivent suivre la langue choisie.
  const FEATURES = [
    { Icon: AudioIcon, label: t('home_feature_audio') },
    { Icon: ModulesIcon, label: `${MODULES.length} ${t('home_feature_modules_suffix')}` },
  ];
          ...
          <View style={styles.sheet}>
            ...
            <Animated.View style={{ opacity: buttonFade, transform: [{ scale: buttonScale }] }}>
              ...
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={t('home_btn_lulu')}
              >
              ...
              >
                <UiIcon name="noSignal" size={16} color={colors.accent} />
